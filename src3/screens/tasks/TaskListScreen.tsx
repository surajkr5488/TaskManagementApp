...
export const TaskListScreen: React.FC<any> = ({ navigation }) => {
  ...
  const TaskItem = ({ item, index }: { item: Task; index: number }) => {
    ...
    const getGradientColors = () => {
      ...
      return [theme.colors.primary || '#6366F1', theme.colors.secondary || '#8B5CF6'];
    };
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme?.colors?.background || '#F9FAFB' }]}>
      ...
      <LinearGradient
        colors={[theme?.colors?.primary || '#6366F1', theme?.colors?.secondary || '#8B5CF6']}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 0 }}
        style={styles.header}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={[styles.emptySubtext, { color: theme?.colors?.secondary || '#9CA3AF' }]}>
